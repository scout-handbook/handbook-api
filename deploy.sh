...
URLS=$(echo "    urls: [$(ls -d */ | grep -v "vendor" | awk -v ORS= '{print "{url: \"https://raw.githubusercontent.com/GenaBitu/OdyMaterialy/master/API/" $1 "/swagger.yaml\", name: \"Version " substr($1, 2) "\"}, "}' | sed 's/..$//')],")
cd swagger-ui
npm install
